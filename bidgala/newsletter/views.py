...
def NewsLetter(request):
        if request.method == 'POST':
            if request.POST.get('art_digest_email_value'):
                post = NewsletterUser()
                post.email = request.POST.get('art_digest_email_value')
                post.save()
...
# def NewsLetter(request):
#         if request.method == 'POST':
#             task=request.POST.get('art_digest_email_value')
#             new = NewsletterUser(email=task)
#             new.save()
#             return render(request,"pages/index.html")